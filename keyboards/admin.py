...
def back_to_admin_keyboard() -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="⬅️ Back to Admin", callback_data="admin:menu")
    return builder.as_markup()
...
def products_list_keyboard(products: list[Product]) -> InlineKeyboardMarkup:
    ...
    for product in products:
        ...
        builder.button(text=f"{status} {product.name}", callback_data=f"admin:products:view:{product.id}")
...
def product_detail_keyboard(product: Product) -> InlineKeyboardMarkup:
    builder = InlineKeyboardBuilder()
    builder.button(text="✏️ Name", callback_data=f"admin:products:edit:{product.id}:name")
    builder.button(text="✏️ Description", callback_data=f"admin:products:edit:{product.id}:description")
    builder.button(text="✏️ Price", callback_data=f"admin:products:edit:{product.id}:price")
    builder.button(text="✏️ Stock", callback_data=f"admin:products:edit:{product.id}:stock")
...
def categories_list_keyboard(categories: list[Category]) -> InlineKeyboardMarkup:
    ...
    for category in categories:
        ...
        builder.button(text=f"{status} {category.name}", callback_data=f"admin:categories:delete:{category.id}")
...
def category_delete_confirm_keyboard(category_id: int) -> InlineKeyboardMarkup:
    ...
    builder.button(text="❌ Cancel", callback_data="admin:categories")
    builder.adjust(1)
    return builder.as_markup()
...
def order_detail_keyboard(order: Order) -> InlineKeyboardMarkup:
    ...
    builder.button(text="↩️ Refunded", callback_data=f"admin:orders:status:{order.id}:refunded")
